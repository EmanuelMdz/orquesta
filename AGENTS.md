...
This is a local TypeScript/Node CLI, chat skill and MCP server. By default Astra plans, answers decisions, creates independent tests and reviews; Opus proposes implementation files. Users can change the provider/model assigned to each role. Preserve the division between implementation and independent quality, and the rule that an approval cannot override failed tests.
