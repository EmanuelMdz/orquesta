...
export const defaults:Config={version:1,orchestratorProvider:'codex',implementerProvider:'claude',astraModel:'gpt-6-astra',opusModel:'opus',workers:2,maxQuestions:3,maxCalls:40,timeoutMs:300000,agentTimeoutMs:0,agentIdleTimeoutMs:900000,maxContextBytes:160000,qaRoot:'test/orquesta',qaCommand:['node','--test'],checks:[],instructions:'',setupCommands:[]};
export function validateConfig(input:unknown):Config{
  if(!input||typeof input!=='object'||Array.isArray(input))throw new Error('La configuración debe ser un objeto.');
  const allowed=new Set([...Object.keys(defaults),'codexPath','claudePath','maxCorrections']);
  for(const key of Object.keys(input))if(!allowed.has(key))throw new Error('Campo de configuración desconocido: '+key);
  // Ignore the retired field in old project configs, presets and run snapshots.
  const {maxCorrections:_legacyCorrections,...current}=input as Record<string,unknown>;
  const config={...structuredClone(defaults),...current} as Config;
  ...
  for(const [key,min,max] of [['workers',1,4],['maxQuestions',1,10],['maxCalls',1,200],['timeoutMs',1000,1800000],['agentTimeoutMs',0,86400000],['agentIdleTimeoutMs',0,86400000],['maxContextBytes',1000,1000000]] as const){if(!Number.isInteger(config[key])||config[key]<min||config[key]>max)throw new Error('Configuración inválida: '+key);}
