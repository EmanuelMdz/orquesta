...
export async function startServer(engine:Engine,options:{port?:number;demo?:boolean}={}){
  ...
  const server=createServer(async(request,response)=>{
    ...
    try{
      ...
      if(target.pathname.startsWith('/api/')){
        ...
        if(request.method==='GET'&&target.pathname==='/api/state'){send(response,200,{repo:engine.repo,demo:!!options.demo,running:engine.running,runs:engine.store.list().map(publicRun)});return;}
