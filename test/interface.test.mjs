...
test('dashboard loads API data, switches tabs, selects agents and records user answers safely',async t=>{
  ...
  t.after(async()=>{dom.window.close();await server.close();await engine.close();rmSync(root,{recursive:true,force:true,maxRetries:5,retryDelay:200});});
  ...
  w.TextDecoder=TextDecoder;w.eval(readFileSync(new URL('../public/app.js',import.meta.url),'utf8'));
  await until(()=>w.document.getElementById('goal').textContent==='Objetivo de prueba');
